 ...
 * @property {string|undefined}  test-cmd    The name of the command in the package's "scripts" object to run the project's tests (default "test").
 * @property {string|undefined}  token       An auth token for Sonarqube, if required.
 *
...
function getCommandLineArguments() {
	...
	opts.getHelpText = function getHelpText() {
		return `
--help      (alias: -h):  Display this help content.
...
--test-cmd:               The name of the command in the package's "scripts" object to run the project's tests (default "test").
--token     (alias: -t):  An auth token for Sonarqube, if required.
