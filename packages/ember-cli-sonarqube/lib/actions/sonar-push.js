...
function buildSonarConfig(config) {
	...
	if (typeof config.cmdOpts.token === 'string') {
		properties.token = config.cmdOpts.token;
	}

	if (config.pkg.repository) {
		...
		else if (typeof config.pkg.repository.url === 'string') {
			properties.options['sonar.links.homepage'] = config.pkg.repository.url;
			properties.options['sonar.links.scm'] = config.pkg.repository.url;
		}
	}
	...
	if (config.files.testResultFile) {
		properties.options['sonar.testExecutionReportPaths'] = config.files.testResultFile;
	}
